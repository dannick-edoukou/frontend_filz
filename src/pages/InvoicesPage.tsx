  ...
  return <>
    ...
    <section className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
      <article className="overflow-hidden rounded-2xl border border-line bg-white">
        <div className="flex items-center gap-3 border-b border-line p-5">
          ...
          <div>
            ...
            <p className="text-xs text-ink-faint">Les justificatifs sont disponibles pendant 24 mois.</p>
          </div>
        </div>
            ...
            <tbody>
              {loading ? (
                <tr className="border-t border-line text-sm">
                  <td colSpan={5} className="px-5 py-8 text-center text-xs text-ink-faint">
                ...
                invoices.map((invoice) => (
                  <tr key={invoice.id} className="border-t border-line text-sm">
                    <td className="px-5 py-4 font-bold">{invoice.id}</td>
                    <td className="px-4 py-4 text-ink-soft">{invoice.date}</td>
                    <td className="px-4 py-4 font-semibold">{invoice.amount}</td>
                    <td className="px-4 py-4">
                      {invoice.state === "paid" ? (
                        <span className="rounded-full bg-teal-50 px-2.5 py-1 text-[11px] font-bold text-teal-700">Payée</span>
                      ) : invoice.state === "pending" ? (
                        <span className="rounded-full bg-pine-50 px-2.5 py-1 text-[11px] font-bold text-pine-600">En attente</span>
                      ) : (
                        <span className="rounded-full bg-clay-100 px-2.5 py-1 text-[11px] font-bold text-clay-500">Échouée</span>
                      )}
                    </td>
                    <td className="px-5 py-4 text-right">
                      <button
                        disabled={invoice.state !== "paid"}
                        className="focus-ring inline-flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs font-bold text-pine-700 hover:bg-pine-50 disabled:opacity-40 disabled:hover:bg-transparent"
      ...
      <aside className="space-y-5">
        <section className="rounded-2xl bg-pine-950 p-5 text-white">
          <p className="text-xs font-bold uppercase tracking-[.14em] text-gold-300">Prochain paiement</p>
          <p className="mt-3 text-3xl font-bold tracking-[-.05em]">
            {loading ? "…" : formatAmountFCFA(planPrice)}
          </p>
          <p className="mt-2 text-sm text-pine-100">
            {loading
              ? "Chargement…"
              : nextDate
          ...
          <Button className="mt-5 !bg-white !text-ink hover:!bg-sand">Voir l’abonnement</Button>
        </section>
