...
export function SuperadminShell({ screen, onNavigate, children }: SuperadminShellProps) {
  ...
  return <div className="min-h-screen bg-paper text-ink">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[274px] flex-col bg-pine-950 px-4 py-5 transition-transform lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <nav aria-label="Navigation administration Filz" className="mt-9 space-y-1"><p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[.18em] text-pine-100/50">Plateforme</p>{adminNavigation.map((item) => <button key={item.id} onClick={() => navigate(item.id)} className={`focus-ring flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold ${screen === item.id ? "bg-pine-800 text-gold-300" : "text-pine-100/80 hover:bg-white/5 hover:text-paper"}`}><HugeiconsIcon icon={item.icon} size={19} />{item.label}</button>)}</nav>
      ...
      <div className="mt-4 flex items-center gap-3 rounded-xl bg-white/5 p-3"><span className="grid h-8 w-8 place-items-center rounded-full bg-gold-400 text-xs font-bold text-pine-950">MK</span><div><p className="text-xs font-bold text-paper">Mariam Koné</p><p className="text-[11px] text-pine-100/60">Superadmin Filz</p></div></div>
    </aside>
