...
export function Button({ className = "", children, variant = "primary", icon, size = "md", type = "button", ...props }: ButtonProps) {
  const variants = {
    primary: "bg-pine-900 text-white hover:bg-pine-800 shadow-primary",
    secondary: "bg-card text-ink border border-line hover:border-pine-200 hover:bg-sand",
    ghost: "text-ink-soft hover:bg-sand",
    danger: "bg-clay-100 text-clay-700 hover:bg-clay-200"
  };
  const sizes = {
...
export function PageHeader({ eyebrow, title, description, action }: {eyebrow?: string;title: string;description: string;action?: ReactNode;}) {
  return <header className="mb-7 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:items-end sm:justify-between">
    <div>
      {eyebrow && <p className="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-pine-500">{eyebrow}</p>}
...
export function MetricCard({ label, value, helper, tone = "orange", icon }: {label: string;value: string;helper: string;tone?: "orange" | "green" | "blue" | "charcoal";icon?: ReactNode;}) {
  ...
  return <article className="rounded-2xl border border-line bg-card p-5 shadow-card">
